...
RequestCallback::HandleSuccess(const nsACString &aURL) {
  ...
  jsval result = createResult(aURL, (nsACString *)nullptr, 0, true, true, true);
...
ContentPermissionRequest::GetPrincipal(nsIPrincipal **aPrincipal) {
  ...
  return mManager->GetPrincipal(aPrincipal);
}
...
ContentPermissionRequest::Allow() {
  ...
  nsresult rv = mManager->RequestURL(mManager->mWAToken,
...
nsDOMPushManager::GetCurrentRequestURL(const nsACString &aToken,
  ...
  nsresult rv = GetManifestURL(getter_Copies(manifestURL));
  NS_ENSURE_SUCCESS(rv, rv);
...
nsresult
nsDOMPushManager::GetManifestURL(char **aManifestURL) {
...
nsresult
nsDOMPushManager::UnregisterPushNotification() {
  nsCString manifestURL;
  nsresult rv = GetManifestURL(getter_Copies(manifestURL));
  NS_ENSURE_SUCCESS(rv, rv);

  nsCOMPtr<nsIPushNotification> service = getPushNotificationService();
  nsCOMPtr<nsIPushNotificationRequest> request;
  rv = service->UnregisterWA(manifestURL, getter_AddRefs(request));
  NS_ENSURE_SUCCESS(rv, rv);
  // Just drop the request.  No matter success or failed.
...
nsresult
nsDOMPushManager::GetPrincipal(nsIPrincipal **aPrincipal) {
  nsCOMPtr<nsPIDOMWindow> window = do_QueryReferent(mWindow);
  NS_ASSERTION(window, "lost the reference to the window.");
